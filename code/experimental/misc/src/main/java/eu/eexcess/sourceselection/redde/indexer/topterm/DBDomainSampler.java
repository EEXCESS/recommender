...
public class DBDomainSampler extends TopTermToWNDomain {
	...
	public static class SampleArguments {
		 ...
		 */
		public Set<TreeNode<String>> sampleDomains;
	}
	...
	private TreeNode<String> domainToTermsTree;
	...
	public TreeNode<String> alignTerms(int fromTopTermIndex, int toTopTermIndex) throws Exception {
	...
	<T> Set<T> distinctUnifyValues(Set<TreeNode<T>> trees) {
		final HashSet<T> unified = new HashSet<T>();

		TreeNode.NodeInspector<T> operator = (n) -> unified.addAll(n.getValues());
		for (TreeNode<T> tree : trees) {
			TreeNode.depthFirstTraverser(tree, operator);
		}
		return unified;
	}
