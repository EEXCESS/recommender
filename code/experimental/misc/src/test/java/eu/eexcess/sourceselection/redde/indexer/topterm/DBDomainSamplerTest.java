	...
	public void alignTerms_expectNotExceptional() {
		if (Settings.isResourceAvailable(Settings.BaseIndex) && Settings.isWordNet20ResourceAvailable()
			...
			try {
				...
				TreeNode.NodeInspector<String> counter = (n) -> {
					numNodes.incrementAndGet();
					numTerms.set(numTerms.get() + n.getValues().size());
				};

				TreeNode.depthFirstTraverser(domainToTerms, counter);
	...
	public void sampleDatabase_expectNotExceptoinal() {
		if (Settings.isResourceAvailable(Settings.BaseIndex) && Settings.isWordNet20ResourceAvailable()
			...
			try {
				...
				TreeNode.NodeInspector<String> printer = (n) -> {
					if (n.getValues().size() > 0) {
						System.out.print("#terms: " + n.getValues().size() + " ");
						System.out.println(n.toString());
					}
				};
				TreeNode.depthFirstTraverser(domainToTermTree, printer);
