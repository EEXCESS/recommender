...
public class SerendiptiyDecomposer implements
	...
	public SecureUserProfileEvaluation decompose(
		...
		for (History history : historyList) {
			inputSecureUserProfile.contextKeywords.add(new ContextKeyword(history.title,0.2,true));
		}
		java.util.List<Interest> interestList= inputSecureUserProfile.interestList;
		for (Interest interest : interestList) {
			inputSecureUserProfile.contextKeywords.add(new ContextKeyword(interest.text,0.4,true));
