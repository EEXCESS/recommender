...
public class ANDCG {
   /**
    * calculates the initial gain vector based on the judgment if the document contains the actual query and the
    * alpha value.
    * @param numElements
    * @param alpha
    * @param results
    ...
    * @return
    */
	private List<Double> calculateGainVector(int numElements, double alpha,
			List<Result> results, List<String> nuggets) {
		...
		System.out.println(gainVector);
		return gainVector;
	}
    
	private int sumNuggedJuge(List<String> nuggets, List<Result> results,
		int k) {
		int sum=0;
		for(int i=0;i<=k;i++){
	 ...
	 * @return
	 */
	private List<Double> calculateDiscountedCumulativGain(
			List<Double> gainVector) {
	...
	/**
	 * Calculates the NormalizedDiscountCumulativGain for the given result list and the given "nuggets"
	 * were the "nuggets" represent the query terms.
	 * If alpha (between 0.1 and 1.0) is higher diversity is rewared more. (0.5 seems to be a good choice )
	 * @param alpha
	 * @param documents
	 * @param nuggets
	 * @return
	 ...
	 * @return
	 */
	private List<Double> normalizedDiscountCumulativGain(
			List<Double> discountedCumulativGain) {
		// TODO: add function
