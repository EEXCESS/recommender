...
public class PartnersDomainsProbe {
	...
	private Logger logger = Logger.getLogger(PartnersDomainsProbeTest.class);
	...
	public Map<PartnerBadge, HashSet<PartnerDomain>> probePartners() throws DomainDetectorException {
		...
		for (int i = 0; i < maxWords; i++) {
			...
			logger.info("searching for ambiguous phrase '" + ambiguousPhrase + "' (" + (i + 1) + "/" + maxWords + ")");
			...
			for (PartnerBadge partner : partnerRegistration.getPartners()) {
				...
				for (Result result : partnerResult.results.subList(0,
								Math.min(maxResults, partnerResult.results.size()))) {
					...
					if (result.title != null && !result.title.trim().isEmpty()) {
						...
						if (!seenResults.contains(titleNormalised)) {
							...
							logger.info("domains for '" + title + "' -> '" + detectedDomains + "'");
			...
			for (Entry<PartnerBadge, TermSet<TypedTerm>> e : partnerToDomainProbes.entrySet()) {
	...
	private Map<PartnerBadge, HashSet<PartnerDomain>> getProbes(Map<PartnerBadge, TermSet<TypedTerm>> partnerProbes) {
		...
		Map<PartnerBadge, HashSet<PartnerDomain>> partnerDomains = new HashMap<PartnerBadge, HashSet<PartnerDomain>>(
						partnerProbes.size());
