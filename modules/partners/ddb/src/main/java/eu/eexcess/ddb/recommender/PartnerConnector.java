...
public class PartnerConnector extends PartnerConnectorBase implements PartnerConnectorApi {
    ...
    public Document queryPartner(PartnerConfiguration partnerConfiguration, SecureUserProfile userProfile, PartnerdataLogger logger) throws IOException {
        ...
        // final String url =
        // "https://api.deutsche-digitale-bibliothek.de/items/OAXO2AGT7YH35YYHN3YKBXJMEI77W3FF/view";
        ...
        /*
         * // get JSON data via HTTP request header authentication String
         * httpJsonResult = httpGet(url, new HashMap<String, String>() { {
         ...
         * "UTF-8"); String queryJsonResult = httpGet(queryJsonURL, null);
        ...
        // Client(PartnerConfigurationEnum.CONFIG.getClientJacksonJson());
        ...
        // DDBResponse ddbResponse = mapper.readValue(httpJSONResult,
        // DDBResponse.class);

        /*
         * JAXBContext jaxbContext = JAXBContext.newInstance(DDBDocument.class);
         * Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
         * ZBWDocument zbwResponse = (DDBDocument)
         * jaxbUnmarshaller.unmarshal(respStringReader); for (ZBWDocumentHit hit
         * : zbwResponse.hits.hit) { try{
         */
        /*
         * WebResource service = client.resource(searchRequest); ObjectMapper
         * mapper = new ObjectMapper(); Builder builder =
         * service.accept(MediaType.APPLICATION_JSON); EuropeanaResponse
         * response= builder.get(EuropeanaResponse.class); if
         * (response.items.size() > numResultsRequest) response.items =
         * response.items.subList(0, numResultsRequest);
         * PartnerdataTracer.dumpFile(this.getClass(), partnerConfiguration,
         * response.toString(), "service-response",
         * PartnerdataTracer.FILETYPE.JSON); client.destroy(); if
         * (makeDetailRequests) { HashMap<EuropeanaDoc, Future<Void>> futures=
         * new HashMap<EuropeanaDoc, Future<Void>>(); final
         * HashMap<EuropeanaDoc, EuropeanaDocDetail> docDetails= new
         * HashMap<EuropeanaDoc,EuropeanaDocDetail>(); final
     ...
     * EEXCESSDataTransformationException { try { Client client = new
     * Client(PartnerConfigurationEnum.CONFIG.getClientJacksonJson());
     * Map<String, String> valuesMap = new HashMap<String, String>();
     * valuesMap.put("objectId", objectId); valuesMap.put("apiKey",
     * partnerConfiguration.apiKey); String detailEndpoint =
     * "http://europeana.eu/api/v2/record/${objectId}.json?wskey=${apiKey}";
     * String detailRequest = StrSubstitutor.replace(detailEndpoint, valuesMap);
     * WebResource service = client.resource(detailRequest); Builder builder =
     * service.accept(MediaType.APPLICATION_JSON); EuropeanaDocDetail
     * jsonResponse = builder.get(EuropeanaDocDetail.class);
     * PartnerdataTracer.dumpFile(this.getClass(), partnerConfiguration,
     * jsonResponse.toString(), "detail-response",
     * PartnerdataTracer.FILETYPE.JSON); client.destroy(); return jsonResponse;
    ...
    public Document queryPartnerDetails(PartnerConfiguration partnerConfiguration, DocumentBadge document, PartnerdataLogger logger) throws IOException {
        ...
        try {
            ...
            String httpXMLResult = callDDBAPI(key, searchRequest, "application/xml"); // print
                                                                                      // results
