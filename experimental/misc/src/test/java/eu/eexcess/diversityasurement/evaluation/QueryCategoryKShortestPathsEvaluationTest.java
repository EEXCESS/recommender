...
public class QueryCategoryKShortestPathsEvaluationTest {
	...
	public void writeToIndexTest_reconstructFromBinary_expectCorrectValues() throws IOException, ParseException,
		...
		CategoryRelevanceDetails crd = new CategoryRelevanceDetails();
		crd.query = "query";
		crd.documenId = 99;
		...
		crd.topCategoryRelevances = new CategoryRelevanceDetails.TopCategoryRelevance[2];
		crd.topCategoryRelevances[0] = new CategoryRelevanceDetails.TopCategoryRelevance();
		...
		crd.topCategoryRelevances[1] = new CategoryRelevanceDetails.TopCategoryRelevance();
		...
		QueryCategoryKShortestPathsEvaluation q = new QueryCategoryKShortestPathsEvaluation();
		q.openOutIndex();
		q.writeToIndex(crd);
		q.closeOutIndex();
		...
		IndexReader ir;
		try {
			ir = DirectoryReader.open(FSDirectory.open(q.fileResource.outLuceneIndexDirectory));
		} catch (IOException e) {
		...
		TopCategoryRelevance tcr = (TopCategoryRelevance) ois.readObject();
		...
		tcr = (TopCategoryRelevance) ois.readObject();
