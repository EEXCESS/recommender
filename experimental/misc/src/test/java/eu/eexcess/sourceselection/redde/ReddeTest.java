        ...
        if (ReddeSettings.isResourceAvailable(ReddeSettings.testSets()) && ReddeSettings.isWordNet30ResourceAvailable()) {
            ...
            while (true) {
                ...
                try {
                    ...
                    System.out.println("some sample indices are empty [" + e.getMessage() + "] -> resample/re-test [" + maxTestRetry + "]");
                    e.printStackTrace();
                    if (maxTestRetry-- <= 0) {
                        break;
                    }
                ...
                System.out.println("est. centralized complete db size = " + estimator.estimatedTotalDocumentsOfCentralizedCompleteDB());
                    ...
                    for (Map.Entry<String, HashSet<String>> entry : keyWords.entrySet()) {
                        for (String query : entry.getValue()) {
                            for (String source : setsUnderTest) {
                                ...
                                System.out.println("{filtered [" + relevance.documents.numberScoredFiltered + "/" + relevance.documents.numberScoredCentralized
                                        + "] while centralized index contains [" + relevance.documents.numberCentralized + "]} = " + relevance.relevance);
                            }
                        }
                    }
        ...
        if (ReddeSettings.isResourceAvailable(ReddeSettings.testSets()) && ReddeSettings.isWordNet30ResourceAvailable()) {
            ...
            try {
                ...
                for (QueryRelevance queryRelevance : ranks) {
                    System.out.println(queryRelevance.indexName + " - " + queryRelevance.relevanceDistribution + "/" + queryRelevance.relevance + "ratio "
                            + queryRelevance.probabilityOfRelevanceRatio + "");
                }
